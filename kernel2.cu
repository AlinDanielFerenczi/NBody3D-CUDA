...
extern "C"
{
    DllExport cudaError_t calculatePosition2(Rigidbody* positions, Vector3D* velocities, Vector3D* accelerations, float* masses, float timeStep, int size);
    cudaError_t calculatePosition2(Rigidbody* positions, Vector3D* velocities, Vector3D* accelerations, float* masses, float timeStep, int size);
}

__global__ void calculateVelocityKernel2(Vector3D* velocities, Vector3D* accelerations, float timeStep)
{
...
__global__ void calculatePositionKernel2(Rigidbody*positions, Vector3D *velocities, float timeStep)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    float multipliedX = velocities[i].X * timeStep + positions[i].position.X;
    float multipliedY = velocities[i].Y * timeStep + positions[i].position.Y;
    float multipliedZ = velocities[i].Z * timeStep + positions[i].position.Z;
    ...
    positions[i].position = result;
}

__global__ void calculateAccelerationKernel2(Rigidbody* positions, float* masses, Vector3D* accelerations)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int j = blockIdx.y * blockDim.y + threadIdx.y;

    float diffX = positions[j].position.X - positions[i].position.X;
    float diffY = positions[j].position.Y - positions[i].position.Y;
    float diffZ = positions[j].position.Z - positions[i].position.Z;
...
__global__ void initAccelerationsKernel2(Vector3D* accelerations)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    Vector3D newVector;
    newVector.X = 0;
...
cudaError_t calculatePosition2(Rigidbody* positions, Vector3D* velocities, Vector3D* accelerations, float* masses, float timeStep, int size)
{
    ...
    cudaStatus = cudaMalloc((void**)&dev_positions, size * sizeof(Rigidbody));
    if (cudaStatus != cudaSuccess) {
    ...
    cudaStatus = cudaMemcpy(dev_positions, positions, size * sizeof(Rigidbody), cudaMemcpyHostToDevice);
    if (cudaStatus != cudaSuccess) {
    ...
    calculateVelocityKernel2 <<<simpleGrid, block>>> (dev_velocities, dev_accelerations, timeStep);
    calculatePositionKernel2 <<<simpleGrid, block>>> (dev_positions, dev_velocities, timeStep);
    initAccelerationsKernel2 <<<simpleGrid, block>>> (dev_accelerations);
    calculateAccelerationKernel2 <<<grid, block>>> (dev_positions, dev_masses, dev_accelerations);
    calculateVelocityKernel2 <<<simpleGrid, block>>> (dev_velocities, dev_accelerations, timeStep);
    ...
    cudaStatus = cudaMemcpy(positions, dev_positions, size * sizeof(Rigidbody), cudaMemcpyDeviceToHost);
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMemcpy failed!");
        goto Error;
    }
