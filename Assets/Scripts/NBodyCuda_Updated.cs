...
public class NBodyCuda_Updated : MonoBehaviour
{
    [DllImport(@"D:\Faculty\PDPCourse\NBody\x64\Debug\NBody.dll", 
        EntryPoint = "calculatePosition")]
    private static extern Vector3[] calculatePosition2(
        Rigidbody[] position, 
        Vector3[] velocity, 
    ...
    void Start()
    {
        ...
        for (int i = 0; i < numberOfBodies; i++)
        {
            ...
            bodies[i] = Instantiate(prefab, new Vector3(
                GetRandomNumber(-2, 2),
                GetRandomNumber(-2, 2),
                GetRandomNumber(-2, 2)
            ), Quaternion.identity);
    ...
    private void FixedUpdate()
    {
        ...
        transform.position = new Vector3(
            bodies.Select(body => body.position.x).Average(),
            bodies.Select(body => body.position.y).Average(),
            bodies.Select(body => body.position.z).Average()
        );
    ...
    private void ComputeAccelerations()
    {
        for (int i = 0; i < numberOfBodies; i++)
        {
            ...
            for (int j = 0; j < numberOfBodies; j++)
            {
                ...
                var diff = bodies[j].position - bodies[i].position;
